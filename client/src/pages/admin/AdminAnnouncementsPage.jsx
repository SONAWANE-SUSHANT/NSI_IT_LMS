...
export default function AdminAnnouncementsPage() {
  ...
  // Data for Course / Batch selectors
  ...
  const [formData, setFormData] = useState({
    title: '',
    message: '',
    scope: 'GLOBAL', // 'GLOBAL' | 'COURSE' | 'BATCH'
    course_id: '',
    batch_id: '',
    status: 'DRAFT',
  });
  ...
  const loadData = async () => {
    try {
      ...
      const [annRes, courseRes, batchRes] = await Promise.all([
        fetchAdminAnnouncements({ status: statusFilter || undefined, search: searchTerm || undefined }),
        getCourses().catch(() => ({ data: [] })),
        fetch(`${API_BASE_URL}/admin/batches`, { headers }).then((r) => r.json()).catch(() => ({ data: [] })),
      ]);
  ...
  const handleOpenEditModal = (ann) => {
    ...
    let scope = 'GLOBAL';
    if (ann.batch_id) scope = 'BATCH';
    else if (ann.course_id) scope = 'COURSE';
  ...
  const handleFormSubmit = async (e, forcePublish = false) => {
    ...
    if (formData.scope === 'COURSE' && !formData.course_id) {
      setFormError('Please select a target course');
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        course_id: formData.scope === 'GLOBAL' ? null : Number(formData.course_id) || null,
        batch_id: formData.scope === 'BATCH' ? Number(formData.batch_id) || null : null,
      ...
      if (editingAnnouncement) {
        ...
        setSuccessMsg(
          forcePublish
            ? 'Announcement published and notifications delivered!'
  ...
  const handlePublish = async (ann) => {
    if (!window.confirm(`Publish "${ann.title}" now? Intended students will immediately receive notifications.`)) {
      return;
    }
    try {
      await publishAdminAnnouncement(ann.id);
  ...
  const handleArchive = async (ann) => {
    if (!window.confirm(`Archive "${ann.title}"? Students will no longer see it as active.`)) {
      return;
    }
    try {
      await archiveAdminAnnouncement(ann.id);
  ...
  const handleDelete = async (ann) => {
    if (!window.confirm(`Permanently delete "${ann.title}"? This cannot be undone.`)) {
      return;
    }
    try {
  ...
  const filteredBatchesForCourse = formData.course_id
    ? batches.filter((b) => String(b.course_id) === String(formData.course_id))
    : batches;
  ...
  return (
    <div className="space-y-6 pb-12 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Publish academy announcements, schedule changes, and alerts to students.
          </p>
        </div>
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-xs sm:text-sm font-bold text-white shadow-md hover:shadow-lg transition-all cursor-pointer w-full sm:w-auto min-h-[42px]"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-white rounded-2xl border border-slate-200/80 p-4 sm:p-5 shadow-xs transition-all hover:shadow-sm">
          <span className="text-[11px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider">Total Notices</span>
          <div className="text-2xl sm:text-3xl font-black text-slate-900 mt-1">{totalCount}</div>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200/80 p-4 sm:p-5 shadow-xs transition-all hover:shadow-sm">
          <span className="text-[11px] sm:text-xs font-bold text-emerald-600 uppercase tracking-wider">Published</span>
          <div className="text-2xl sm:text-3xl font-black text-emerald-600 mt-1">{publishedCount}</div>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200/80 p-4 sm:p-5 shadow-xs transition-all hover:shadow-sm">
          <span className="text-[11px] sm:text-xs font-bold text-amber-600 uppercase tracking-wider">Drafts</span>
          <div className="text-2xl sm:text-3xl font-black text-amber-600 mt-1">{draftCount}</div>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200/80 p-4 sm:p-5 shadow-xs transition-all hover:shadow-sm">
          <span className="text-[11px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider">Archived</span>
          <div className="text-2xl sm:text-3xl font-black text-slate-600 mt-1">{archivedCount}</div>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center px-4">
            ...
            <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
              Create announcements to notify students of schedule changes, materials, or general news.
            </p>
            <button
              type="button"
              onClick={handleOpenCreateModal}
          ...
          <>
            {/* Desktop / Tablet Enterprise Table (hidden on mobile) */}
                    ...
                    return (
                      <tr key={ann.id} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="py-4 px-4 whitespace-nowrap">
                          ...
                          {isCourse && (
                            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[11px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 max-w-[200px] truncate" title={`Course: ${ann.course?.code || ann.course?.name}`}>
                              <BookOpen className="w-3 h-3 shrink-0" />
                              <span className="truncate">Course: {ann.course?.code || ann.course?.name}</span>
                            </span>
                          )}
                          {isBatch && (
                            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[11px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 max-w-[200px] truncate" title={`Batch: ${ann.batch?.name}`}>
                              <Users className="w-3 h-3 shrink-0" />
                              <span className="truncate">Batch: {ann.batch?.name}</span>
                            </span>
                          )}
                        </td>
                        ...
                        {/* Created By */}
                        <td className="py-4 px-4 whitespace-nowrap text-slate-600 font-medium text-[11px]">
                          {ann.creator
                            ? `${ann.creator.first_name || ''} ${ann.creator.last_name || ''}`.trim() || ann.creator.email
                            : 'Admin'}
                        </td>
            ...
            {/* Mobile / Compact Intentional Cards (hidden on desktop) */}
                  ...
                  <div key={ann.id} className="p-4 space-y-3">
                    ...
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      <div className="min-w-0">
                        ...
                        {isCourse && (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[11px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 truncate max-w-[200px]" title={`Course: ${ann.course?.code || ann.course?.name}`}>
                            <BookOpen className="w-3 h-3 shrink-0" />
                            <span className="truncate">Course: {ann.course?.code || ann.course?.name}</span>
                          </span>
                        )}
                        {isBatch && (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[11px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 truncate max-w-[200px]" title={`Batch: ${ann.batch?.name}`}>
                            <Users className="w-3 h-3 shrink-0" />
                            <span className="truncate">Batch: {ann.batch?.name}</span>
                          </span>
                        )}
                    ...
                    {/* Info Row: Published Date & Created By */}
                    ...
                    <div className="pt-2 border-t border-slate-100 flex items-center gap-2 flex-wrap">
                      ...
                      <button
                        ...
                        onClick={() => handleDelete(ann)}
                        title="Delete"
                        className="min-h-[38px] px-3 py-1.5 rounded-xl text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 transition-colors inline-flex items-center justify-center gap-1.5 cursor-pointer font-bold text-xs"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                        <span>Delete</span>
                      </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white rounded-3xl max-w-xl w-full p-4 sm:p-6 shadow-2xl border border-slate-200/90 relative max-h-[90vh] overflow-y-auto">
            {/* Modal Header */}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Target by academy, course, or specific batch.
                  </p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={(e) => handleFormSubmit(e, false)} className="space-y-4 pt-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Schedule Update: Live Class Postponed"
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  {[
                    { id: 'GLOBAL', label: 'Academy Wide', icon: Globe },
                    { id: 'COURSE', label: 'Course Scope', icon: BookOpen },
                    { id: 'BATCH', label: 'Batch Scope', icon: Users },
                  ].map((s) => {
                    const Icon = s.icon;
                      ...
                      <button
                        ...
                        onClick={() =>
                          setFormData({
                            ...formData,
                            scope: s.id,
                            course_id: s.id === 'GLOBAL' ? '' : formData.course_id,
                            batch_id: s.id === 'BATCH' ? formData.batch_id : '',
                          })
                        }
              ...
              {/* Course Selector (if COURSE or BATCH scope) */}
              ...
              <div className="pt-3 border-t border-slate-100 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2.5">
                <button
                  type="button"
                  onClick={handleCloseModal}
                  className="w-full sm:w-auto min-h-[40px] px-4 py-2.5 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-100 transition-colors cursor-pointer text-center"
                >
                  Cancel
                </button>
                ...
                <button
                  ...
                  className="w-full sm:w-auto min-h-[40px] px-4 py-2.5 rounded-xl text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200 transition-colors cursor-pointer disabled:opacity-50 text-center"
                >
                  {modalSubmitting ? 'Saving...' : 'Save as Draft'}
                </button>
                ...
                <button
                  ...
                  className="w-full sm:w-auto min-h-[40px] px-5 py-2.5 rounded-xl text-xs font-bold text-white shadow-md hover:shadow-lg transition-all cursor-pointer disabled:opacity-50 inline-flex items-center justify-center gap-1.5"
                  style={{ background: ADMIN_PRIMARY }}
                >
                  <Send className="w-3.5 h-3.5" />
